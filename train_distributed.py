...
def train_text_adapter_distributed(
    rank,
    world_size,
    ...
    for epoch in range(start_epoch, text_epoch):
        ...
        train_loader.sampler.set_epoch(epoch)  # Important for distributed training
        loss_list = []
        
        for input_data in tqdm(train_loader, disable=rank != 0):
            ...
            with torch.no_grad():
                ...
                cls_token, _ = adapted_model.module.clipmodel.encode_image(image, [])
                cls_token = cls_token / cls_token.norm(dim=-1, keepdim=True)
                patch_features = [
                    clip_surgery.module.visual.ln_post(t[:, 1:, :]) for t in patch_features
                ]
                patch_features = [t @ clip_surgery.module.visual.proj for t in patch_features]
                patch_features = [
        ...
        avg_loss = torch.tensor(np.mean(loss_list)).to(device)
        ...
        if rank == 0:
            ...
            torch.save(
                {
                    "epoch": epoch + 1,
                    "text_adapter": adapted_model.module.text_adapter.state_dict(),
...
def train_image_adapter_distributed(
    rank,
    world_size,
    model: nn.Module,
    ...
    for epoch in range(start_epoch, image_epoch):
        ...
        if rank == 0:
            ...
            model_dict = {
                "epoch": epoch + 1,
                "image_adapter": model.module.image_adapter.state_dict(),
...
def main_worker(rank, world_size, args):
    ...
    if rank == 0:
        ...
        logging.basicConfig(
            filename=os.path.join(args.save_path, "train_distributed.log"),
            encoding="utf-8",
            level=logging.INFO,
        )
    ...
    clip_surgery = DDP(clip_surgery, device_ids=[rank], output_device=rank)
    clip_model = DDP(clip_model, device_ids=[rank], output_device=rank)
    
    model.eval()
    
    # Setup optimizers
    text_optimizer = torch.optim.Adam(
        model.module.text_adapter.parameters(),
        lr=args.text_lr,
        betas=(0.5, 0.999),
    )
    image_optimizer = torch.optim.Adam(
        model.module.image_adapter.parameters(),
        lr=args.image_lr,
        betas=(0.5, 0.999),
    )
    ...
    if len(text_file) > 0:
        ...
        model.module.text_adapter.load_state_dict(checkpoint["text_adapter"])
    ...
    if len(file) > 0:
        ...
        model.module.image_adapter.load_state_dict(checkpoint["image_adapter"])
    ...
    text_sampler = DistributedSampler(text_dataset, num_replicas=world_size, rank=rank)
    image_sampler = DistributedSampler(image_dataset, num_replicas=world_size, rank=rank)
    
    text_dataloader = torch.utils.data.DataLoader(
        text_dataset, batch_size=args.text_batch_size, sampler=text_sampler, **kwargs
    )
    ...
    image_dataloader = torch.utils.data.DataLoader(
        image_dataset, batch_size=args.image_batch_size, sampler=image_sampler, **kwargs
    )
    
    # Training
    if adapt_text:
        model = train_text_adapter_distributed(
            rank=rank,
    ...
    model = train_image_adapter_distributed(
        rank=rank,
        world_size=world_size,
        model=model,
